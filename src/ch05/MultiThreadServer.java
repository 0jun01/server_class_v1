...
public class MultiThreadServer {
	...
	public static void main(String[] args) {
		...
		try (ServerSocket serverSocket = new ServerSocket(5000)) {
			// 클라이언트 대기 ---> 연결 요청이 오면 -- 소켓 객체를 생성(클라이언트와 연결된 상태)
			...
			// 키보드 스트림 준비
			BufferedReader keboardReader = new BufferedReader(new InputStreamReader(System.in));
			
			// 스레드를 시작합니다.
			startReadThread(readerStream); // <--- main join() 대기 상태
			startWriteThread(writerStream, keboardReader);
